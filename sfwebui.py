...
from sflib import Arskiyfoot

from sfscan import startArskiyfootScanner

from Arskiyfoot import ArskiyfootDb
from Arskiyfoot import ArskiyfootHelpers
from Arskiyfoot import __version__
from Arskiyfoot.logger import logListenerSetup, logWorkerSetup

mp.set_start_method("spawn", force=True)


class ArskiyfootWebUi:
    """Arskiyfoot web interface."""
    ...
    def __init__(self: 'ArskiyfootWebUi', web_config: dict, config: dict, loggingQueue: 'logging.handlers.QueueListener' = None) -> None:
        """Initialize web server.

        Args:
            web_config (dict): config settings for web interface (interface, port, root path)
            config (dict): Arskiyfoot config
            loggingQueue: TBD
        ...
        dbh = ArskiyfootDb(self.defaultConfig, init=True)
        sf = Arskiyfoot(self.defaultConfig)
        ...
        self.log = logging.getLogger(f"Arskiyfoot.{__name__}")
    ...
    def error_page(self: 'ArskiyfootWebUi') -> None:
        """Error page."""
    ...
    def error_page_401(self: 'ArskiyfootWebUi', status: str, message: str, traceback: str, version: str) -> str:
    ...
    def error_page_404(self: 'ArskiyfootWebUi', status: str, message: str, traceback: str, version: str) -> str:
        ...
        """
        templ = Template(filename='Arskiyfoot/templates/error.tmpl', lookup=self.lookup)
        return templ.render(message='Not Found', docroot=self.docroot, status=status, version=__version__)

    def jsonify_error(self: 'ArskiyfootWebUi', status: str, message: str) -> dict:
    ...
    def error(self: 'ArskiyfootWebUi', message: str) -> None:
    ...
    def cleanUserInput(self: 'ArskiyfootWebUi', inputList: list) -> list:
    ...
    def searchBase(self: 'ArskiyfootWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search.
    ...
    def buildExcel(self: 'ArskiyfootWebUi', data: list, columnNames: list, sheetNameIndex: int = 0) -> str:
    ...
    @cherrypy.expose
    def scanexportlogs(self: 'ArskiyfootWebUi', id: str, dialect: str = "excel") -> bytes:
        """Get scan log
        ...
        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename=Arskiyfoot-{id}.log.csv"
        cherrypy.response.headers['Content-Type'] = "application/csv"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return fileobj.getvalue().encode('utf-8')

    @cherrypy.expose
    def scancorrelationsexport(self: 'ArskiyfootWebUi', id: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if scan_name:
                fname = f"{scan_name}-Arskiyfoot-correlations.xlxs"
            else:
                fname = "Arskiyfoot-correlations.xlxs"
        ...
        if filetype.lower() == 'csv':
            ...
            if scan_name:
                fname = f"{scan_name}-Arskiyfoot-correlations.csv"
            else:
                fname = "Arskiyfoot-correlations.csv"
    ...
    @cherrypy.expose
    def scaneventresultexport(self: 'ArskiyfootWebUi', id: str, type: str, filetype: str = "csv", dialect: str = "excel") -> str:
    ...
    @cherrypy.expose
    def scaneventresultexportmulti(self: 'ArskiyfootWebUi', ids: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if len(ids.split(',')) > 1 or scan_name == "":
                ...
                fname = scan_name + "-Arskiyfoot.xlsx"
        ...
        if filetype.lower() == 'csv':
            ...
            if len(ids.split(',')) > 1 or scan_name == "":
                ...
                fname = scan_name + "-Arskiyfoot.csv"
    ...
    @cherrypy.expose
    def scansearchresultexport(self: 'ArskiyfootWebUi', id: str, eventType: str = None, value: str = None, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            cherrypy.response.headers['Content-Disposition'] = "attachment; filename=Arskiyfoot.xlsx"
            cherrypy.response.headers['Content-Type'] = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            cherrypy.response.headers['Pragma'] = "no-cache"
            return self.buildExcel(rows, ["Updated", "Type", "Module", "Source",
                                   "F/P", "Data"], sheetNameIndex=1)
        ...
        if filetype.lower() == 'csv':
            ...
            cherrypy.response.headers['Content-Disposition'] = "attachment; filename=Arskiyfoot.csv"
            cherrypy.response.headers['Content-Type'] = "application/csv"
            cherrypy.response.headers['Pragma'] = "no-cache"
            return fileobj.getvalue().encode('utf-8')
    ...
    @cherrypy.expose
    def scanexportjsonmulti(self: 'ArskiyfootWebUi', ids: str) -> str:
        ...
        if len(ids.split(',')) > 1 or scan_name == "":
            fname = "Arskiyfoot.json"
        else:
            fname = scan_name + "-Arskiyfoot.json"
    ...
    @cherrypy.expose
    def scanviz(self: 'ArskiyfootWebUi', id: str, gexf: str = "0") -> str:
        ...
        if gexf == "0":
            return ArskiyfootHelpers.buildGraphJson([root], data)

        if not scan_name:
            fname = "Arskiyfoot.gexf"
        else:
            fname = scan_name + "Arskiyfoot.gexf"

        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename={fname}"
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return ArskiyfootHelpers.buildGraphGexf([root], "Arskiyfoot Export", data)

    @cherrypy.expose
    def scanvizmulti(self: 'ArskiyfootWebUi', ids: str, gexf: str = "1") -> str:
        ...
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return ArskiyfootHelpers.buildGraphGexf(roots, "Arskiyfoot Export", data)

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanopts(self: 'ArskiyfootWebUi', id: str) -> dict:
    ...
    @cherrypy.expose
    def rerunscan(self: 'ArskiyfootWebUi', id: str) -> None:
        """Rerun a scan.
        ...
        targetType = ArskiyfootHelpers.targetTypeFromString(scantarget)
        if not targetType:
            ...
            # target type again.
            targetType = ArskiyfootHelpers.targetTypeFromString(f'"{scantarget}"')

        if targetType not in ["HUMAN_NAME", "BITCOIN_ADDRESS"]:
            scantarget = scantarget.lower()

        # Start running a new scan
        scanId = ArskiyfootHelpers.genScanInstanceId()
        try:
            p = mp.Process(target=startArskiyfootScanner, args=(self.loggingQueue, scanname, scanId, scantarget, targetType, modlist, cfg))
            p.daemon = True
            p.start()
    ...
    @cherrypy.expose
    def rerunscanmulti(self: 'ArskiyfootWebUi', ids: str) -> str:
        """Rerun scans.
        ...
        for id in ids.split(","):
            ...
            if targetType is None:
                ...
                return self.error("Invalid target type. Could not recognize it as a target Arskiyfoot supports.")
        ...
        templ = Template(filename='Arskiyfoot/templates/scanlist.tmpl', lookup=self.lookup)
        return templ.render(rerunscans=True, docroot=self.docroot, pageid="SCANLIST", version=__version__)

    @cherrypy.expose
    def newscan(self: 'ArskiyfootWebUi') -> str:
        ...
        templ = Template(filename='Arskiyfoot/templates/newscan.tmpl', lookup=self.lookup)
        return templ.render(pageid='NEWSCAN', types=types, docroot=self.docroot,
                            modules=self.config['__modules__'], scanname="",
                            selectedmods="", scantarget="", version=__version__)

    @cherrypy.expose
    def clonescan(self: 'ArskiyfootWebUi', id: str) -> str:
    ...
    @cherrypy.expose
    def index(self: 'ArskiyfootWebUi') -> str:
        """Show scan list page.
    ...
    @cherrypy.expose
    def scaninfo(self: 'ArskiyfootWebUi', id: str) -> str:
        ...
        templ = Template(filename='Arskiyfoot/templates/scaninfo.tmpl', lookup=self.lookup, input_encoding='utf-8')
        return templ.render(id=id, name=html.escape(res[0]), status=res[5], docroot=self.docroot, version=__version__,
                            pageid="SCANLIST")

    @cherrypy.expose
    def opts(self: 'ArskiyfootWebUi', updated: str = None) -> str:
        ...
        """
        templ = Template(filename='Arskiyfoot/templates/opts.tmpl', lookup=self.lookup)
    ...
    @cherrypy.expose
    def optsexport(self: 'ArskiyfootWebUi', pattern: str = None) -> str:
        ...
        cherrypy.response.headers['Content-Disposition'] = 'attachment; filename="Arskiyfoot.cfg"'
        cherrypy.response.headers['Content-Type'] = "text/plain"
        return content

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def optsraw(self: 'ArskiyfootWebUi') -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scandelete(self: 'ArskiyfootWebUi', id: str) -> str:
        """Delete scan(s).
    ...
    @cherrypy.expose
    def savesettings(self: 'ArskiyfootWebUi', allopts: str, token: str, configFile: 'cherrypy._cpreqbody.Part' = None) -> None:
        ...
        if configFile and configFile.file:
            try:
                ...
                return self.error(f"Failed to parse input file. Was it generated from Arskiyfoot? ({e})")
    ...
    @cherrypy.expose
    def savesettingsraw(self: 'ArskiyfootWebUi', allopts: str, token: str) -> str:
        """Save settings, also used to completely reset them to default.
    ...
    def reset_settings(self: 'ArskiyfootWebUi') -> bool:
    ...
    @cherrypy.expose
    def resultsetfp(self: 'ArskiyfootWebUi', id: str, resultids: str, fp: str) -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def eventtypes(self: 'ArskiyfootWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def modules(self: 'ArskiyfootWebUi') -> list:
        """List all modules.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def correlationrules(self: 'ArskiyfootWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def ping(self: 'ArskiyfootWebUi') -> list:
        """For the CLI to test connectivity to this server.

        Returns:
            list: Arskiyfoot version as JSON
        """
        return ["SUCCESS", __version__]

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def query(self: 'ArskiyfootWebUi', query: str) -> str:
    ...
    @cherrypy.expose
    def startscan(self: 'ArskiyfootWebUi', scanname: str, scantarget: str, modulelist: str, typelist: str, usecase: str) -> str:
        """Initiate a scan.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def stopscan(self: 'ArskiyfootWebUi', id: str) -> str:
        """Stop a scan.
        ...
        for scan_id in ids:
            ...
            if scan_status != "RUNNING" and scan_status != "STARTING":
                return self.jsonify_error('400', f"The running scan is currently in the state '{scan_status}', please try again later or restart Arskiyfoot.")
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlog(self: 'ArskiyfootWebUi', id: str, limit: str = None, rowId: str = None, reverse: str = None) -> list:
        """Scan log data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanerrors(self: 'ArskiyfootWebUi', id: str, limit: str = None) -> list:
        """Scan error data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlist(self: 'ArskiyfootWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanstatus(self: 'ArskiyfootWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scansummary(self: 'ArskiyfootWebUi', id: str, by: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scancorrelations(self: 'ArskiyfootWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresults(self: 'ArskiyfootWebUi', id: str, eventType: str = None, filterfp: bool = False, correlationId: str = None) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresultsunique(self: 'ArskiyfootWebUi', id: str, eventType: str, filterfp: bool = False) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def search(self: 'ArskiyfootWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search scans.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanhistory(self: 'ArskiyfootWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanelementtypediscovery(self: 'ArskiyfootWebUi', id: str, eventType: str) -> dict:
        ...
        del pc['ROOT']
        retdata['tree'] = ArskiyfootHelpers.dataParentChildToTree(pc)
